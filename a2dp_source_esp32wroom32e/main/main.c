...
#define BUZZER_FREQ_HZ 1000 // 2 kHz tone
...
static volatile bool s_a2dp_pending = false;  // ADD THIS
static volatile bool s_bt_initialized = false;
static volatile bool s_a2dp_connected = false;
static volatile bool s_a2dp_playing = false;
...
static int32_t bt_app_a2d_data_cb(uint8_t *data, int32_t len)
{
    ...
    int32_t sample_count = len / (sizeof(int16_t) * TONE_CHANNELS);
    generate_tone(samples, (size_t)sample_count);
    return len; // tell A2DP we filled 'len' bytes
}
...
static void bt_app_a2d_cb(esp_a2d_cb_event_t event, esp_a2d_cb_param_t *param)
{
    ...
    case ESP_A2D_CONNECTION_STATE_EVT:
        if (param->conn_stat.state == ESP_A2D_CONNECTION_STATE_CONNECTED) {
            ...
            s_a2dp_pending = false;   // connection done, clear pending
        } else {
            s_a2dp_connected = false;
            s_a2dp_playing = false;
            s_a2dp_pending = false;   // reset on disconnect too
        }
        break;
    ...
    case ESP_A2D_MEDIA_CTRL_ACK_EVT:
        ...
        ESP_LOGI(TAG, "A2DP media ctrl ack: cmd=%d status=%d", cmd, status);
        s_a2dp_pending = false;       // command cycle complete, clear pending
        if (cmd == ESP_A2D_MEDIA_CTRL_CHECK_SRC_RDY &&
            status == ESP_A2D_MEDIA_CTRL_ACK_SUCCESS) {
            s_a2dp_pending = true;    // mark START as in-flight
...
{
    if (!s_a2dp_connected) {
        if (!s_a2dp_pending) {          // don't spam connect attempts
            s_a2dp_pending = true;
            bt_connect_to_peer();
        }
        return;
    ...
    if (!s_a2dp_playing && !s_a2dp_pending) {  // guard here too
        s_a2dp_pending = true;
        ESP_LOGI(TAG, "Checking A2DP source ready...");
